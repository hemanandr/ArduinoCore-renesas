...
#else
   R_SPI_CalculateBitrate(12000000, &(_esp_host_spi_ext_cfg.spck_div));
#endif
...
float I2V(uint16_t x, uint8_t range)
{
   float _vref = 4.096, out_min, out_max;
   switch (range)
   {
   case RNG_B_10_24:
      out_min = -2.5 * _vref;
...
uint8_t write_register(uint8_t reg, uint8_t value)
{
   uint8_t tx_buffer[3] = {(reg << 1) | 0x01, value, 0x00};
   uint8_t rx_buffer[3] = {0, 0, 0};

   R_IOPORT_PinWrite(NULL, ESP_CS, BSP_IO_LEVEL_HIGH);
   R_IOPORT_PinWrite(NULL, ADC_CS, BSP_IO_LEVEL_LOW);
   R_BSP_SoftwareDelay(100, BSP_DELAY_UNITS_MICROSECONDS);
   fsp_err_t err = R_SPI_WriteRead(&_esp_host_spi_ctrl, tx_buffer, rx_buffer, 3, SPI_BIT_WIDTH_8_BITS);
   R_BSP_SoftwareDelay(100, BSP_DELAY_UNITS_MICROSECONDS);
...
uint16_t read_channel(uint8_t channel){
   {
      ...
      fsp_err_t err = R_SPI_WriteRead(&_esp_host_spi_ctrl, tx_buffer, rx_buffer, 4, SPI_BIT_WIDTH_8_BITS);

      R_BSP_SoftwareDelay(100, BSP_DELAY_UNITS_MICROSECONDS);
      R_IOPORT_PinWrite(NULL, ADC_CS, BSP_IO_LEVEL_HIGH);
   }
   {
      uint8_t tx_buffer[4] = {0x00, 0x00, 0x00, 0x00};
      uint8_t rx_buffer[4] = {0, 0, 0, 0};
...
void adc_read(){
   ...
   flow = max(0.0, flow_v - 1.0) * (25.0 / 4.0);
}